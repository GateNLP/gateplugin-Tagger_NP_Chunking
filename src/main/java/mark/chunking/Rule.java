...
public class Rule implements Serializable {
	...
	/**
	 * The new chunk tag that is assigned if this rule matches the input text.
	 **/
	private String outTag = null;

	/**
	 * The full line of the rules file which created this rule, used mainly for
	 * the toString() method.
	 **/
	...
	public Rule(String rule) {
		...
		while (mt.find()) {
			...
			while (mo.find()) {
				...
				Integer offset = Integer.valueOf(mo.group().replaceAll("_", "-"));
	...
	/**
	 * Simply returns true if this rule matches against the sentence at a given
	 * position. This method makes no alterations to the tags assigned to any
	 * specific offset.
	 * 
	 * @param currentToken
	 *            the index within the sentece of the token upon which the rule
	 *            is centered.
	 * @param words
	...
	public boolean match(int currentToken, List<String> words,
		...
		for (int i = 0; i < types.size(); ++i) {
			...
			boolean matchOffset = working.get(currentToken + offset).equals(
					value);
